...
2. Grep `corpus.jsonl` theo số hiệu **và** theo cụm từ nội dung. Nếu router trượt, mở
   ...
   **Có Agent tool** → giao việc grep này cho một subagent rẻ (`model: haiku` hoặc
   `sonnet`), brief tự chứa: kiểu ca, các số hiệu cần tra, cụm từ cần tìm, đường dẫn
   corpus. Nhận về nguyên văn các dòng grep được rồi tự chọn. Tra cứu là việc cơ học,
   không cần model phán đoán. **Không có Agent tool** → tự grep như trên.
