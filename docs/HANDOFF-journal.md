...
- Spec: GitHub issue **#5** (canhtd/principle). Tickets = sub-issues with native blocking:
  ...
  - **#7 (retitled)** 3-column shell + Day view → NEXT. Prototype done and **frozen** at `docs/design/proto-day-A.html` (v2) + `proto-day-A-v2*.png` (A chosen over B, decisions listed in `docs/design/proto-day-README.md`); branch `feat/day-shell` off `main`; next = Swift by Opus 5 xhigh, scope = shell + Day view + column 1 + column 3 **without the dots** (those are #8). Core addition needed: scheduled time on Task.
